...
namespace Awesome.Library.Cryptography {
	...
	/// <summary>
	/// RC2 Cryptographic Algorithm
	/// </summary>
	public class RC2Crypto : IEncryptor {

		#region " Methods "

		byte[] IEncryptor.Encrypt( string password, byte[] buffer ) {
			return RC2Crypto.Encrypt( Encoding.UTF8.GetBytes( password ), buffer );
		}
		...
		byte[] IEncryptor.Encrypt( byte[] password, byte[] buffer ) {
			return RC2Crypto.Encrypt( password, buffer );
		}
		...
		private static byte[] Decrypt( byte[] password, byte[] input_buffer ) {
			if ( input_buffer.Length > 0 ) {
				...
				rc2.IV = rc2_IV;
				//des.GenerateIV();
				CryptoStream encStream = new CryptoStream( memStream, rc2.CreateDecryptor(), CryptoStreamMode.Write | CryptoStreamMode.Read );
				int block_size = (int)( rc2.BlockSize / 8 );
				// encyption here
				for ( int i = 0; i < input_buffer.Length; i += block_size )
					if ( i + block_size <= input_buffer.Length ) {
